...
visualize_temperature <- function(temp_data, city, save_path = NULL) {
  ...
  plots_temp <- lapply(1:length(unique(temp_data$Variable)), function(i) {
    ...
    # Create the plot with a bottom legend
    plot <- plot_ly(temp_subset, x = ~date, y = ~Temperature, color = ~factor(SSP),
                    colors = ssp_colors, linetype = ~Variable) %>%
      add_lines() %>%
      layout(
        title = paste(city, "-", unique(temp_subset$Variable)[1]), 
...
visualize_precipitation <- function(precip_data, city, save_path = NULL) {
  ...
  print(paste("Unique Variables:", unique_variables))  # Debugging line
  
  # Create separate line plots for each Variable
  plots_precip <- lapply(1:length(unique_variables), function(i) {
    precip_subset <- precip_data %>%
      filter(Variable == unique_variables[i])
    print(paste("Processing Variable:", unique_variables[i]))  # Debugging line
    
    # Create the plot
    plot <- plot_ly(precip_subset, x = ~date, y = ~Precipitation, color = ~factor(SSP),
                    colors = ssp_colors, linetype = ~factor(SSP)) %>%
      add_lines() %>%
      layout(
        title = paste("Precipitation in", city, "-", unique_variables[i]),
        xaxis = list(title = "Month", tickformat = "%b"),  # Display abbreviated month names
