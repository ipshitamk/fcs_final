...
visualize_temperature <- function(temp_data, city, save_path = NULL) {
  # Convert 'time' column to Date format, assuming 'time' column is now dates only
  temp_data$date <- as.Date(temp_data$time, format = "%m/%d/%Y")
  
  # Define colors for the two specific SSP values
  ssp_colors <- c("245" = "blue", "585" = "red")
  ...
  plots_temp <- lapply(1:length(unique(temp_data$Variable)), function(i) {
    ...
    plot <- plot_ly(
      ...
      x = ~date,
      y = ~Temperature,
      color = ~factor(SSP, levels = c("245", "585")),  # Only "245" and "585" in the legend
      colors = ssp_colors
    ) %>%
      add_lines() %>%
      layout(
        title = list(
          text = paste0(city, "<br>", unique(temp_subset$Variable)[1]),  # Add line break in title
          font = list(size = 10, family = 'inherit')  # Smaller title text
        ),
        xaxis = list(
          title = "Month",
          tickformat = "%b",
          tickfont = list(family = 'inherit')  # Use CSS-defined font
        ),
        yaxis = list(
          title = "Temperature",
          tickfont = list(family = 'inherit')  # Use CSS-defined font
        ...
        legend = list(
          ...
          x = 0.5, xanchor = "center",  # Center legend at the bottom
          y = -0.2,
          font = list(size = 10, family = 'inherit')  # Small legend font
        )
      )
    ...
    if (!is.null(save_path)) {
      ...
      htmlwidgets::saveWidget(plot, file.path(save_path, paste0("plot_", i, ".html")), selfcontained = FALSE)
    }
...
visualize_precipitation <- function(precip_data, city, save_path = NULL) {
  # Convert `time` column to Date format based on "M/D/YY" format
  precip_data$date <- as.Date(precip_data$time, format = "%m/%d/%y")
  
  # Define specific colors for SSP 245 and SSP 585
  ssp_colors <- c("245" = "blue", "585" = "red")
  
  # Get unique Variables
  unique_variables <- unique(precip_data$Variable)
  print(paste(" ", unique_variables))  # Debugging line
  
  # Create separate line plots for each Variable
  plots_precip <- lapply(1:length(unique_variables), function(i) {
    precip_subset <- precip_data %>%
      filter(Variable == unique_variables[i])
    print(paste(" ", unique_variables[i]))  # Debugging line
    ...
    plot <- plot_ly(
      ...
      layout(
        title = list(
          text = paste0(city, "<br>", unique_variables[i]),
          font = list(size = 10, family = 'inherit')  # Smaller text and inherit font
        ),
        xaxis = list(
          title = "Month",
          tickformat = "%b",
          tickfont = list(family = 'inherit')  # Inherit font
        ),
        yaxis = list(
          title = "Precipitation",
          tickfont = list(family = 'inherit')  # Inherit font
        ...
        legend = list(
          ...
          y = -0.2,
          font = list(family = 'inherit')  # Inherit font
        )
      )
    
    # Save each plot as an HTML file with a letter name if save_path is provided
    if (!is.null(save_path)) {
      dir.create(save_path, showWarnings = FALSE)
      file_name <- file.path(save_path, paste0("plot_", letters[i], ".html"))
      saveWidget(plot, file_name, selfcontained = FALSE)  # Lightweight HTML
    }
...
visualize_cyclones <- function(output, country, city) {
  # Define cyclone categories and thresholds
  category_labels <- c("Cat 1", "Cat 2", "Cat 3", "Cat 4")
  category_thresholds <- c(32, 42, 49, 58)  # Wind speed thresholds (m/s)
  
  # Prepare the data
  output_climate <- reshape2::melt(output[, !(colnames(output) %in% 'Current')], id.vars = 'RPs')
  colnames(output_climate) <- c('RPs', 'GCM', 'WindSpeed')
  
  # Add uncertainty bounds (10% above and below)
  output_climate$Upper <- output_climate$WindSpeed * 1.1
  output_climate$Lower <- output_climate$WindSpeed * 0.9
  
  # Plot setup
  plot <- plot_ly()
  
  # Add uncertainty range (shaded area)
  plot <- plot %>%
    add_trace(
      data = output_climate,
      x = ~RPs,
      ...
      fill = 'tonexty',  # Fill between lines
      fillcolor = 'rgba(255, 0, 0, 0.2)',  # Semi-transparent red
      name = "Climate change (2015-2050)"
    ) %>%
    add_trace(
      data = output_climate,
      x = ~RPs,
  ...
  # Add current wind speed line
  plot <- plot %>%
    add_lines(
      data = output,
      x = ~RPs,
      y = ~Current,
      name = "Current (1980-2017)",
      line = list(color = "blue", width = 2)
    )
  
  # Add cyclone categories on the right Y-axis as annotations
  for (i in seq_along(category_labels)) {
    if (category_thresholds[i] <= max(output_climate$Upper)) {
      plot <- plot %>%
        add_annotations(
          x = max(output$RPs) * 1.1,  
          y = category_thresholds[i],
          text = category_labels[i],
          showarrow = FALSE,
          xref = "paper",  # Anchor the annotation to the plot area
          x = 1.02,        # Position near the right edge
  ...
  plot <- plot %>%
    layout(
      xaxis = list(
        title = "Return Period",
        tickvals = c(10, 100, 1000, 10000),  # Define specific ticks
        ticktext = c("10", "100", "1,000", "10,000"),  # Labels for the ticks
        type = "log",  # Logarithmic scale
        titlefont = list(size = 10)
      ),
      yaxis = list(
        title = "Wind Speed (m/s)",  # Wind speed on the left
        titlefont = list(size = 10)
      ),
      legend = list(
        orientation = "v",
...
roads_exposed <- function(csv_path, year) {
  ...
  # Handle case when no data is available for the specified year
  if (nrow(data_year) == 0) {
    message(paste("No data available for Year", year))
    return(NULL)
  }
  
  # Select specified columns for flood percentages
  selected_columns <- data_year[, c("Return.period", "SSP",
                                    "Percentage.of.road.flooded.more.than.15cm",
                                    "Percentage.of.road.flooded.more.than.30cm",
                                    "Percentage.of.road.flooded.more.than.50cm",
                                    "Percentage.of.road.flooded.more.than.100cm")]
  ...
  # Define factor levels and labels for flood depth categories
  long_data$FloodDepthCategory <- factor(long_data$FloodDepthCategory,
                                         levels = c("Percentage.of.road.flooded.more.than.15cm",
                                                    "Percentage.of.road.flooded.more.than.30cm",
                                                    "Percentage.of.road.flooded.more.than.50cm",
                                                    "Percentage.of.road.flooded.more.than.100cm"),
                                         labels = c(">15cm", ">30cm", ">50cm", ">100cm"))
  
  # Get unique SSPs
  ssps <- unique(long_data$SSP)
  
  # Define custom color palette for plot
  bar_colors <- RColorBrewer::brewer.pal(4, "Blues")  # Adjusted to match the categories
  
  # Create an empty list to store Plotly graphs
  plotly_graphs <- list()
  
  for (ssp in ssps) {
    # Filter data for each SSP
    ...
    ggplot_graph <- ggplot(ssp_data, aes(x = factor(Return.period), y = Percentage, fill = FloodDepthCategory)) +
      ...
      scale_fill_manual(values = bar_colors, name = " ") +
      labs(title = paste("Roads Exposed to Floods in", year, "for", ssp),
           x = "Return Period",
           y = "Percentage") +
      theme_minimal() +
      theme(
        ...
        text = element_text(family = "inherit"),
    ...
    # Store plotly graph in the list
    plotly_graphs[[ssp]] <- plotly_graph
  }
  
  # Return the list of Plotly graphs
  return(plotly_graphs)
}


pop_exposed <- function(csv_path, year, ssp) {
  # Read the CSV file
  pop_exposure1 <- read.csv(csv_path, stringsAsFactors = FALSE)
  
  # Clean column names (optional, but ensures consistency)
  colnames(pop_exposure1) <- trimws(colnames(pop_exposure1))
  
  # Filter data for the specific year and SSP
  data_filtered <- pop_exposure1[pop_exposure1$Year == year & pop_exposure1$SSP == ssp, ]
  
  # Check if data exists after filtering
  if (nrow(data_filtered) == 0) {
    stop(paste("No data available for Year", year, "and SSP", ssp))
  }
  
  # Select relevant columns for plotting
  selected_columns <- data_filtered[, c("Return.period", "SSP", "X.Exposed.population")]
  
  # Convert SSP and Return Period to factors for cleaner plotting
  selected_columns$SSP <- as.factor(selected_columns$SSP)
  selected_columns$Return.period <- factor(selected_columns$Return.period, levels = unique(selected_columns$Return.period))
  
  # Create the ggplot graph
  ggplot_graph <- ggplot2::ggplot(selected_columns, aes(x = Return.period, y = X.Exposed.population, fill = SSP)) +
    geom_bar(stat = "identity", position = "dodge") +  # Bar chart
    labs(
      title = paste("Percentage of Population Exposed to Floods in", year, "for", ssp),
      x = "Return Period",
    ...
    theme(
      ...
      legend.box = "horizontal",  # Horizontal alignment for legend items
      legend.title = element_text(hjust = 0.5),  # Center-align legend title
      panel.grid.major = element_line(color = "#eeeeee"),  # Subtle grid lines
      panel.grid.minor = element_blank(),  # No minor grid lines
      plot.title = element_text(size = 10, hjust = 0.5)  # Center and size the title
    )
  ...
  plotly_graph <- plotly::ggplotly(ggplot_graph) %>%
    layout(
      ...
      font = list(family = "inherit")  # Explicitly inherit font from CSS
    )
  
  # Return the interactive plotly graph
  return(plotly_graph)
}


count_points_spatvector <- function(spatvector) {
  if (geomtype(spatvector) != "points") {
    stop("The SpatVector does not contain point data.")
  }
  num_points <- nrow(spatvector)
  cat("Number of points in the SpatVector:", num_points, "\n")
  return(num_points)
}

convert_to_utm <- function(shapefile_path, output_path = NULL) {
  shp <- vect(shapefile_path)
  
  centroid <- spatSample(shp, size = 1, method = "random")
  lon <- geom(centroid)[1]  # Longitude
  lat <- geom(centroid)[2]  # Latitude
  
  utm_zone <- floor((lon + 180) / 6) + 1
  utm_crs <- ifelse(lat >= 0, 
                    paste0("EPSG:", 32600 + utm_zone),  
                    paste0("EPSG:", 32700 + utm_zone))
  
  shp_utm <- project(shp, utm_crs)
  return(shp_utm)
}

# add_infrastructure_layer <- create_layer_function(
#   data = infrastructure_points,
#   yaml_key = "infrastructure-combined",
#   group_id = "infrastructure-combined",
#   params = list(
#     data_variable = "source",
#     palette = c("health" = "red", "school" = "blue", "police" = "green")
#   )
# )
...
# Function to calculate binary zonal statistics with percentage exposed
zonal_stats_binary <- function(points, raster_data, threshold = 1) {
  # Input:
  # points       - sf object (points)
  # raster_data  - RasterLayer object
  # threshold    - Value above which the raster is considered "exposed"
  
  # Check that inputs are valid
  if (!inherits(points, "sf")) {
    stop("The points input must be an sf object.")
  }
  if (!inherits(raster_data, "RasterLayer")) {
    stop("The raster_data input must be a RasterLayer.")
  }
  
  # Transform points to match the CRS of the raster
  if (!st_crs(points) == crs(raster_data)) {
    points <- st_transform(points, crs(raster_data))
  }
  
  # Extract raster values at point locations
  raster_values <- raster::extract(raster_data, as(points, "Spatial"))
  
  # Apply binary condition: 1 if raster_value > threshold, otherwise 0
  exposed <- ifelse(raster_values > threshold, 1, 0)
  
  # Count the number of exposed points
  count_exposed <- sum(exposed, na.rm = TRUE)
  
  # Calculate the percentage of exposed points
  percentage_exposed <- (count_exposed / length(raster_values)) * 100
  
  # Return results as a list
  result <- list(
    total_points = length(raster_values),
    exposed_points = count_exposed,
    non_exposed_points = length(raster_values) - count_exposed,
    percentage_exposed = percentage_exposed
  )
