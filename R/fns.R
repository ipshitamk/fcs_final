...
count_points <- function(shapefile_path) {
  # Read the shapefile
  point_data <- st_read(shapefile_path, quiet = TRUE)
  
  # Check if the geometry type is POINT
  if (!all(st_geometry_type(point_data) == "POINT")) {
    stop("The shapefile does not contain point data.")
  }
  
  # Count the number of points
  num_points <- nrow(point_data)
  
  # Print and return the count
  cat("Number of points in the shapefile:", num_points, "\n")
  return(num_points)
}
